...
class MovieByGenreListViewModel : ViewModel() {
    ...
    fun getMovieList(withGenres: String){
        viewModelScope.launch {
            responseInitial.postValue(RequestState.Loading)
            responseInitial.postValue(handleMovieListResponse(
                repository.getMoviesByGenre(page,withGenres)
            ))
        }
    }
